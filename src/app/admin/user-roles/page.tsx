      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-[#f5c16c]/20 pb-6">
          <div>
            ...
            <p className="text-white/50 mt-2 max-w-xl">Manage user roles. Revocation removes Verified Lecturer role.</p>
          </div>
          <div className="flex gap-4 flex-wrap">
            {(rolesCatalog ?? []).map(rc => {
              ...
              return (
                <div key={rc.id} className="bg-[#1a0b08]/80 border border-[#f5c16c]/20 rounded-lg px-4 py-2 text-center min-w-[120px]">
                  <div className="text-2xl font-bold text-[#f5c16c]">{count}</div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4 bg-black/30 p-2 rounded-xl border border-[#f5c16c]/20">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40" size={18} />
              <input
                type="text"
                placeholder="Search users by name or email..."
                className="w-full bg-black/30 border border-[#f5c16c]/20 focus:border-[#f5c16c]/50 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/40 outline-none transition-all"
                value={search}
                onChange={e => setSearch(e.target.value)}
              />
            </div>
            <Tabs value={activeRole || (roleTabs[0]?.value ?? "")} onValueChange={setActiveRole} className="flex-1">
              <TabsList className="bg-black/30 border border-[#f5c16c]/20 text-white/60">
                {roleTabs.map(tab => (
                  <TabsTrigger key={tab.value} value={tab.value} className="gap-2 data-[state=active]:bg-[#f5c16c]/20 data-[state=active]:text-[#f5c16c]">
                    <span>{tab.label}</span>
                    <span className="rounded-full bg-[#f5c16c]/20 px-2 py-0.5 text-[10px] text-[#f5c16c]">{roleCounts[tab.value] ?? 0}</span>
                  </TabsTrigger>
                ))}
              </TabsList>
          ...
          <div className="space-y-2">
            ...
            {error && <div className="flex items-center gap-2 text-red-400 px-6 py-4">{error}</div>}
            {!loading && !error && pagedUsers.map(user => (
              <div key={user.id} className="group grid grid-cols-12 items-center px-6 py-4 bg-[#1a0b08]/80 border border-[#f5c16c]/20 rounded-xl hover:border-[#f5c16c]/40 transition-all">
                <div className="col-span-4 flex items-center gap-4">
                  <Avatar className="h-10 w-10 border border-[#f5c16c]/30">
                    <AvatarImage src={user.profileImageUrl ?? undefined} />
                    <AvatarFallback className="bg-[#f5c16c]/20 text-[#f5c16c]">{(user.username ?? 'U').slice(0,2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
                ...
                <div className="col-span-6 text-xs text-white/70">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#f5c16c]/20 bg-black/30 text-white/70 hover:bg-[#f5c16c]/10 transition">
                        <span className="text-[11px] font-semibold uppercase tracking-wide">Roles</span>
                        <span className="rounded-full bg-[#f5c16c]/20 px-2 py-0.5 text-[10px] text-[#f5c16c]">{(user.roles ?? []).length}</span>
                        <ChevronDown size={14} className="text-white/40" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="start" className="min-w-[220px] bg-[#1a0b08] border-[#f5c16c]/20 text-white">
                      {(user.roles ?? []).length > 0 ? (
                ...
                <div className="col-span-1">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border bg-emerald-500/20 text-emerald-400 border-emerald-500/30">
                ...
                <div className="col-span-1 flex justify-end">
                  {(activeRole || (roleTabs[0]?.value ?? "")) === 'verifiedlecturer' && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2 px-3 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold uppercase tracking-wide"
                    >
