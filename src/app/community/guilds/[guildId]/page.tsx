...
export default function GuildDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([gRes, myRes, membersRes, pRes]) => {
        ...
        const fallbackMyCount = isMyGuild ? (myRes.data?.memberCount ?? null) : null;
        setMemberCount(directCount ?? fallbackMyCount ?? (gRes.data?.memberCount ?? null));
        // Derive my role if I am a member
        if (authId && Array.isArray(membersRes.data)) {
          const me = (membersRes.data as any[]).find((m) => m.authUserId === authId);
  ...
  const handleApply = async () => {
    ...
    setSubmitting(true);
    try {
      await guildsApi.applyToJoin(guildId as string, { message: joinMessage || null });
  ...
  return (
    <div className="relative max-h-screen w-full overflow-hidden bg-[#08040a] text-foreground">
      ...
      <div className="flex flex-col gap-6 pb-16">
        ...
        {!loading && guild && (
          <Card className="rounded-[28px] border-white/12 bg-gradient-to-br from-[#381c12]/86 via-[#200e11]/93 to-[#0d0508]/97">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-6 text-xs uppercase tracking-[0.35em] text-foreground/50">
                ...
                {guild.isPublic ? (
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1">Public</span>
                ) : (
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1">Invite Only</span>
                )}
              </div>
              {!isMember && (
                <Dialog>
        ...
        {!loading && guild && (
          <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); if (typeof window !== "undefined") window.location.hash = v; }} className="mt-2">
            <TabsList>
            ...
            <TabsContent value="home" className="space-y-4">
              <div className="text-sm text-foreground/70">Welcome to {guild.name}. Use the tabs to navigate.</div>
            </TabsContent>
