...
export const googleMeetApi = {
  ...
  async createSpace(
    ...
    const res = await fetch(`${BASE}/spaces`, { method: "POST", headers: authHeaders(token), body: JSON.stringify(body ?? {}) });
  ...
  async normalizeSpaceName(token: string, nameOrCode: string): Promise<string> {
    ...
    const raw = nameOrCode.startsWith("spaces/")
      ? nameOrCode.slice("spaces/".length)
      : nameOrCode;
    ...
    if (looksLikeMeetingCode) {
      ...
      if (!space?.name)
        throw new Error("Failed to resolve meeting code to space name");
      return space.name; // already in the form "spaces/{space}"
    }
    if (nameOrCode.startsWith("conferenceRecords/")) {
      const record = await this.getConferenceRecord(token, nameOrCode);
      const spaceName = record?.space;
      if (!spaceName)
        throw new Error("Failed to resolve conference record to space name");
      return spaceName;
    }
  ...
  async endActiveConference(token: string, spaceNameOrId: string): Promise<{}> {
    ...
    const res = await fetch(`${BASE}/${name}:endActiveConference`, { method: "POST", headers: authHeaders(token), body: JSON.stringify({}) });
  ...
  async listConferenceRecords(
    ...
    const res = await fetch(`${BASE}/conferenceRecords?${params.toString()}`, { headers: authHeaders(token) });
    if (!res.ok) throw new Error(`List conferences failed: ${res.status}`);
    return res.json();
  },

  async getConferenceRecord(token: string, nameOrId: string) {
    const name = nameOrId.startsWith("conferenceRecords/") ? nameOrId : `conferenceRecords/${nameOrId}`;
    const res = await fetch(`${BASE}/${name}`, { headers: authHeaders(token) });
    if (!res.ok) throw new Error(`Get conference record failed: ${res.status}`);
    return res.json();
  },

  // Participants for a conference
  async listParticipants(token: string, conferenceId: string) {
    const res = await fetch(`${BASE}/conferenceRecords/${conferenceId}/participants`, { headers: authHeaders(token) });
  ...
  async listParticipantSessions(
    ...
    const res = await fetch(`${BASE}/conferenceRecords/${conferenceId}/participants/${participantId}/participantSessions`, { headers: authHeaders(token) });
  ...
  async listRecordings(token: string, conferenceId: string) {
    const res = await fetch(`${BASE}/conferenceRecords/${conferenceId}/recordings`, { headers: authHeaders(token) });
  ...
  async listTranscripts(token: string, conferenceId: string) {
    const res = await fetch(`${BASE}/conferenceRecords/${conferenceId}/transcripts`, { headers: authHeaders(token) });
  ...
  async listTranscriptEntries(
    ...
    const res = await fetch(`${BASE}/conferenceRecords/${conferenceId}/transcripts/${transcriptId}/entries`, { headers: authHeaders(token) });
